#ifndef MODELR1_H_INCLUDED
#define MODELR1_H_INCLUDED
...
class Model{
    ...
    int n,m,p; //Size of Matrices 
    int actions_possible_global; //
     // Number of admissible actions per state
    float discretization; //Total number of shares after discretization
    
    vector<Eigen::VectorXf> all_actions; //Stores all possible model actions
    vector<Eigen::VectorXf> B_time_action;// B*all_actions Used to speed up the initialization of each state's actions
    Eigen::VectorXf noise; 
    function<double(const Eigen::VectorXf&)> cost_function;
    function<Eigen::VectorXf(double)> goal; //Reference
    float discrete_time_step; //temporal discretization of the model
    int actions_state_possible; //number of actions allowed by states
    //Model
    ...
    //Discretize the states
        ...
        Model( int actions_state_possible,const Eigen::MatrixXf& A, const Eigen::MatrixXf& B, const Eigen::MatrixXf& C, const Eigen::MatrixXf& D, const Eigen::MatrixXf& discretization_actions, const Eigen::VectorXf& noise,function<double(const Eigen::VectorXf&)> cost_function, function<Eigen::VectorXf( double)> goal , float discrete_time_step){
            bool ax = true;
        ...
        Eigen::VectorXf deterministic(Eigen::VectorXf x, Eigen::VectorXf u,double current_time){
            ...
            Eigen::VectorXf x_real = A*(goal(current_time)* 1/(discretization) - x)  +B*u;

            return discretizeVector(goal(current_time)* 1/(discretization) - x_real);
        ...
        Eigen::VectorXf n_deterministic(Eigen::VectorXf x, Eigen::VectorXf u,double current_time){
            ...
            Eigen::VectorXf x_real = A*(goal(current_time)* 1/(discretization) - x)  +B*(u + u_r) ;
        ...
        */
        ...
        Eigen::VectorXf insert_all_exploited(unordered_map<Eigen::VectorXf, Node*, MyHash::EigenVectorXfHash, std::equal_to<Eigen::VectorXf>>& all_exploited,Eigen::VectorXf current_state, double current_time ){
            ...
            f_s = this->discretizeVector( next_ - B*(actions->action + u_r));
            auto it_exploited = all_exploited.find(f_s);
            if(it_exploited != all_exploited.end()){
        ...
        void writeDataToCSV(vector<Eigen::VectorXf>& states, const vector<float>& t, const vector<int>& all_trial, const string& filename) {
            ...
            if (file.is_open()) {
                ...
                for ( i = 0; i < numElements; ++i) {
                    ...
                    x_real = goal(t[i]) -  states[i]*discretization;
                    
                    file  <<  x_real .transpose() <<" "<< goal(t[i]).transpose() <<"\t"<< t[i] << "\t" << all_trial[i] << "\n" ;
        ...
        void one_simulation(int o, Eigen::VectorXf incial_state, function<bool(Eigen::VectorXf)> stop_criterion , double mst, int horizon, string NameBase){
            ...
            Eigen::VectorXf current_state = discretizeVector((this->goal(0) - incial_state )*1/discretization);
            ...
            while( !stop_criterion(current_state) && mst>current_time ){
                ...
                // cout << "Para o estado antes da trial:" << current_state.transpose()<< endl;
                
                // for(auto& action: all_exploited[current_state]->action_state_trial){
                //     cout << action.action*discretization << " : "<< action.cost << endl;
                ...
                // cout << "Para o estado depois da trial:" << current_state.transpose()<< endl;
                ...
                current_state = insert_all_exploited(all_exploited, current_state, current_time - discrete_time_step);
                // current_state = n_deterministic(current_state, all_exploited[current_state]->action_state_trial.begin()->action, current_time - discrete_time_step);
                // current_state = all_exploited[current_state]->ExploitationStep(this, all_exploited, current_time - this->discrete_time_step);
